...
var calc_data = function(data) {
    ...
    , record_yds = 5476, byes = 0, tds_broken = 'no',yds_broken = 'no';
    ...
    var tds_per_game = total_tds/(weeks - byes);
    var yds_per_game = total_yds/(weeks - byes);
    if(total_tds > record_tds) {
        var record_week_tds = weeks;
        tds_broken = 'yes';
    }
    else {
//        var record_week_tds = Math.ceil((record_tds + 1)/tds_per_game);
        var record_week_tds = Math.ceil(weeks + ((record_tds + 1 - total_tds)/tds_per_game));

    }
    if(total_yds > record_yds) {
        record_week_yds = weeks;
        yds_broken = 'yes';
    }
    else {
//        record_week_yds = Math.ceil((record_yds + 1)/yds_per_game);
        var record_week_yds = Math.ceil(weeks + ((record_yds + 1 - total_yds)/yds_per_game) );
...
var build_page = function(data,record_week_tds,record_week_yds,tds_per_game,yds_per_game,tds_broken,yds_broken,tds_projected,yds_projected,total_tds,total_yds) {
    ...
    if(record_week_yds > 17) {
        var yds_text = " and not on track to break the passing yards record."; 
    }
    else {
        var yds_text = " and on track to break the passing yards record in week " + record_week_yds + '.';
    }
